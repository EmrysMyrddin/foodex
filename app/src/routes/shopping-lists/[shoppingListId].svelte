...
<Grid>
  {#each recipes as { id: entryId, recipe: { id }, qte } (entryId)}
      <Recipe recipeId={id}>
        <div>x{qte} <span class="set-prepared-icon" on:click|preventDefault={setRecipePrepared(entryId, true)}>✅</span></div>
      </Recipe>
  {/each}
</Grid>
  ...
  {#each preparedRecipes as { id: entryId, recipe: { id }, qte } (entryId)}
    <li>
      <Recipe recipeId={id} {qte}>
        <span class="set-prepared-icon" on:click|preventDefault={setRecipePrepared(entryId, false)}>🚫</span>
      </Recipe>
    </li>
  {/each}
</ul>
